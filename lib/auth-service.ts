...
import {
  EMAIL_VERIFICATION_TOKEN_COOLDOWN_MS,
...
export async function createPendingCredentialsUser(email: string, password: string) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    return { user, tokenId: token.id, rawToken };
  });
}
...
export async function requestEmailVerification(normalizedEmail: string) {
  ...
  if (!user || user.emailVerifiedAt) {
    ...
    console.info(JSON.stringify({ event: "email_verification.request_accepted" }));
    return;
  }
  ...
  const tokenRow = await prisma.$transaction(async (tx) => {
    ...
    const lockedUsers = await tx.$queryRaw<Array<{ email_verified_at: Date | null }>>`
      SELECT "email_verified_at" FROM "users" WHERE "id" = ${user.id} FOR UPDATE
    `;
    if (!lockedUsers[0] || lockedUsers[0].email_verified_at) return null;
    const newest = await tx.emailVerificationToken.findFirst({
    ...
    if (
      newest &&
      now.getTime() - newest.createdAt.getTime() < EMAIL_VERIFICATION_TOKEN_COOLDOWN_MS
    ) {
      return null;
    }
  ...
  if (tokenRow) {
    console.info(
      JSON.stringify({ event: "email_verification.issued", userId: user.id, tokenId: tokenRow.id }),
    );
    try {
      await sendVerificationEmail(user.email, rawToken, tokenRow.id);
    } catch {
      await invalidateEmailVerificationToken(tokenRow.id);
    }
  }
  console.info(JSON.stringify({ event: "email_verification.request_accepted" }));
}

export async function isEmailVerificationTokenActive(rawToken: string) {
