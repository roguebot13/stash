...
describe("auth DAL", () => {
  ...
  it("returns only safe user fields for a current session version", async () => {
    ...
    findUniqueMock.mockResolvedValue({ id: "user-1", email: "person@example.com", sessionVersion: 3, passwordHash: "must-not-leak" });
  ...
  it("rejects a session issued before a version increment", async () => {
    ...
    findUniqueMock.mockResolvedValue({ id: "user-1", email: "person@example.com", sessionVersion: 3 });
